...
const AddDespesa = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch('http://localhost:3100/api/despesas', {
      method: 'POST',
      headers: {
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Erro ao adicionar despesa.');
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      <div className="card p-4 shadow" style={{ maxWidth: '400px', width: '100%' }}>
        <h2 className="text-center mb-4">Adicionar Despesa</h2>
